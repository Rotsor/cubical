...
module LFSetElim {ℓ} {B : LFSet A → Type ℓ}
...
module LFSetRec {ℓ} {B : Type ℓ}
  ...
  f : LFSet A → B
  f = LFSetElim.f []* (λ x xs → x ∷* xs) (λ x y b → comm* x y b) (λ x b → dup* x b) λ _ → trunc*


module LFPropElim {ℓ} {B : LFSet A → Type ℓ}
  ([]* : B []) (_∷*_ : (x : A) {xs : LFSet A} → B xs → B (x ∷ xs))
  (trunc* : (xs : LFSet A) → isProp (B xs)) where

  f : ∀ x → B x
  f = LFSetElim.f []* _∷*_
    (λ x y zs → isOfHLevel→isOfHLevelDep {n = 1} trunc* (x ∷* (y ∷* zs))
                  (y ∷* (x ∷* zs)) (comm _ _ _))
    (λ x ys → isOfHLevel→isOfHLevelDep {n = 1} trunc* _ _ (dup _ _))
